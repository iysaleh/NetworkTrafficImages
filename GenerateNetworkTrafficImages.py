...
def generate_time_relative_ntis(packet_list,nti_x,nti_y,output_dir,base_filename,nti_packet_arrangement,nti_time_range_ms=10000,max_number_ntis_per_file=999999999999,nti_fill_threshold=0.2):
	#nti_packet_arrangement can be 'linear','edge_spiral','center_spiral','waterfall','diagonal'
	#time_range is the maximum size of the NTI_Area
	#If the array is less full than the fill_threshold, don't accept the NTI as a valid NTI
	nti_area = nti_x * nti_y
	cur_packet_list_index = 0
	...
	while(nti_ending_time <= packet_list[len(packet_list)-1].packet_time and number_ntis_generated < max_number_ntis_per_file): #Case: There are enough packets to generate a new NTI
		...
		#Write the 2D array to file (that is the NTI) if and only if 
		if(np.count_nonzero(nti_array)/nti_array.size >= nti_fill_threshold):
